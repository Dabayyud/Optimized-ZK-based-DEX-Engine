...
function accountLeafHash2(account: AccountState): bigint {
    let balanacesHash = 0n
    for (const [token, balances] of account.balances) {
        const tokenHash = poseidon2([
            convertToBigInt(normalizeAddress(token)),
            poseidon2([convertToBigInt(balances.available), convertToBigInt(balances.locked)])
        ])
        balanacesHash = poseidon2([balanacesHash, token])
    }
    return poseidon2([account.nonce, balanacesHash])
}
...
function seedGlobalAccount(user: `0x${string}`, token:`0x${string}`, available: bigint, locked: bigint, nonce: bigint) {
    ...
    balances.set(checksumAddress(token), {
        available: available,
        locked: locked,
    })
    ...
    },
    {
        ...
        account: {
            ...
            balances: new Map ([
                [
                    WETH_ADDRESS,
                    {available: 1000n, locked: 0n}
                ] // the latest one will be the final source
            ])
        }
    }
]

// Logic follows as is: Cannot query data while user info is being updated for the same user thus atomocity remains.
// If using an update with a query on user balance while transaction is in process (for a new user) mapping would not exist thus undefined.
...
const newBalances1 = new Map(currentAccount.balances)
const currentWethBalance: BalanceState  = newBalances1.get(WETH_ADDRESS)! || {available: 0n, locked: 0n} // map already created for user 1 
// IMMUTABILTY PRINCIBLES 

newBalances1.set(WETH_ADDRESS, {
    ...currentWethBalance, // assigning a copy of that data over here to maintain immutability
    available: currentWethBalance.available + 500n
})
...
const update2: AccountUpdate[] = [
    {
        ...
        account: {
            ...
            balances: newBalances1
        }
    }
]
console.log(update2[0])
...
newBalances2.set(WETH_ADDRESS, {
    ...existingWeth2,
    available: existingWeth2.available! + 50n // should now return 1550n (1000n + 500n + 50n)
})
...
executeBatchAccountUpdates([update3[0]]) // 1550n
