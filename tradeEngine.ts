...
export class TradeEngine {
    ...
    async submitOrder(limitOrder: LimitOrder) {
        const isValid = await ValidationGuard.isOrderValid(limitOrder)
        if (!isValid){
    ...
    private validateOrder(limitOrder: LimitOrder) {
        ...
        const targetToken = limitOrder.side === 0 ? limitOrder.sToken : limitOrder.bToken
        const requiredAmount = limitOrder.side === 0 ? limitOrder.amount : limitOrder.price * limitOrder.amount

        if(!accountState.balances.has(targetToken)) {
            accountState.balances.set(targetToken, {available: 0n, locked: 0n})
        }

        const state = accountState.balances.get(targetToken)!

        state.available -= requiredAmount;
        state.locked += requiredAmount;
    }
 ...
 * Change validate order for cold start problem
 * 
 * HOW THE SOLIDITY SMART CONTRACT WILL WORK:
 * 
 * 1) Users deposit funds into the smart contract which emits an event.
 * 2) Our state manager reads this event and updates user balances accordingly.
 * 3) Users then sign an off chain message to execute their order via a WEBSOCKET API.
 * 4) Our L2 will verify this signature and submit an order to the trade engine.
 * 5) Order will be verified and matched and executed. Tree will be updates and a batch is returned + new root and ZK PROOF
 * 6) The generated outputs/proof will be verified by our solidity smart contract and allow users to withdraw based on new state.
 * - Eveytime a user deposits in the smart contract which the watcher service reads and updates the SMT root 
 * - zk proof is emitted and needed everytime so that the old root + the batch = new root. 
 * - there is a specific predetermined amount of gas fees calculated and implemented as a trading fee for withdrawals.
 * - specific call data is needed so that the user has the sibling nodes required to build their merkle proof. 
 * - User calls the sequencer through the websocket API (frontend), signs and requests are through this. 
 */